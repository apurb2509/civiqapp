...
function NotificationBell() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Prevent closing the dropdown if a notification modal is open
  ...
  const notificationCount =
    profile?.role === "admin" ? newReportCount : unreadCount;
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="fixed inset-x-0 bottom-0 sm:absolute sm:top-full sm:right-0 sm:left-auto sm:bottom-auto sm:mt-3 
             w-full sm:w-80 md:w-96 max-h-[85vh] bg-slate-900 border border-slate-700/50 
             rounded-t-2xl sm:rounded-xl shadow-2xl overflow-y-auto backdrop-blur-xl z-[999]"
            >
              {/* Header */}
              ...
              {/* Notifications List */}
              <div className="max-h-[70vh] overflow-y-auto custom-scrollbar px-1 sm:px-2">
                {loading ? (
                  ...
                  <div className="divide-y divide-slate-700/30">
                    {notifications.map((notif, index) => (
                      <motion.div
                        ...
                        className={`group relative bg-gradient-to-br from-slate-800/90 to-slate-900/80 rounded-xl shadow-lg border border-slate-700/50 p-4 sm:p-5 mb-3 cursor-pointer transition-all duration-300 hover:shadow-cyan-500/20 hover:-translate-y-1 ${
                          !notif.is_read ? "ring-1 ring-cyan-500/50" : ""
                        }`}
                      >
                        <div className="flex items-start gap-3">
                          {/* Icon */}
                          <div
                            className={`flex-shrink-0 w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-lg ${
                              !notif.is_read
        ...
        {selectedNotif && (
          <NotificationDetailModal
            ...
            onClose={() => {
              ...
              setIsOpen(true); // reopen the notification panel after closing the detail view
            }}
          />
        )}
      </AnimatePresence>
