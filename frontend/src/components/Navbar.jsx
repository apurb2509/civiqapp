    ...
    <header className="bg-gray-900 bg-opacity-70 backdrop-blur-lg sticky top-0 z-50">
      ...
      {isMenuOpen && (
        <div className="md:hidden px-4 pt-2 pb-4 space-y-2 bg-gray-800">
          <Link to="/" className="block text-gray-300 hover:text-white">{t('navbar.home')}</Link>
          <Link to="/reports" className="block text-gray-300 hover:text-white">{t('navbar.viewReports')}</Link>
          <hr className="border-gray-700"/>
           {user ? (
            <>
              <p className="text-sm text-gray-400 pt-2">{user.email}</p>
              <button onClick={handleSignOut} className="w-full text-left text-red-400 hover:text-red-300">{t('navbar.signOut')}</button>
            </>
          ) : (
            <Link to="/auth" className="block text-gray-300 hover:text-white">{t('navbar.login')}</Link>
          )}
           <div className="pt-2">
